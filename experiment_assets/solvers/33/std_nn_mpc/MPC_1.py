...
class Solve_MPC:
    def __init__(self, Q_1_mat, Q_2_mat, load_min, load_max, list_r_x_pu, list_p_q_pu, load_min_Q, load_max_Q):
        ...
        self.Prices = np.array([Price_low, Price_low, Price_low, Price_low, Price_low, Price_low,
                                ...
                                Price_median, Price_median, Price_median, Price_median, Price_median, Price_median,
                                Price_median, Price_median, Price_median, Price_median, Price_median, Price_median])
        self.standard_nn_path = Path(__file__).resolve().parent / "standard_sac_critics_33.npz"
    ...
    def _terminal_feature_bounds(self, current_time, window_time):
        z0_lb = np.zeros(74, dtype=float)
        z0_ub = np.ones(74, dtype=float)
        terminal_time = window_time - 1 + current_time

        t_normal_st = terminal_time / 47
        z0_lb[0] = t_normal_st
        z0_ub[0] = t_normal_st

        for i in range(32):
            value_p = (
                self.list_p_q_pu_t[terminal_time][i][0] - self.load_min[i]
            ) / (self.load_max[i] - self.load_min[i] + 1e-6)
            value_q = (
                self.list_p_q_pu_t[terminal_time][i][1] - self.load_min_Q[i]
        ...
        pv_value = (
            self.pv_11_avail_list[terminal_time] - self.pv_11_min
        ) / (self.pv_11_max - self.pv_11_min + 1e-6)
        wt_value = (
            self.wind_26_avail_list[terminal_time] - self.wind_26_min
    ...
    def sol_pro(self, current_time, P_DG_5_init, SOC_BSS_9_init, load_list, pv_11_avail_list, wind_26_avail_list,
        ...
        try:
            ...
            model.addGenConstrMin(Q_f, [q1_out, q2_out], name="std_q_min")
            ...
            model.setParam(GRB.Param.TimeLimit, self.time_limit_s)
