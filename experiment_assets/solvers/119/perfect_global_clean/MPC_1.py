...
class Solve_MPC:
    ...
    def sol_pro_all_day(self, window_time,
        ...
        try:
            ...
            lv_ij_i = model.addMVar(shape=(window_time, 1, 118), vtype=GRB.CONTINUOUS, name='lv_ij_i')
            ...
            model.setParam(GRB.Param.TimeLimit, 432)
        ...
        return {
            "obj": float(obj),
            "cost_hour": [float(dict_value[f"cost_hour[{t}]"]) for t in range(window_time)],
            "P_DG_29": [float(dict_value[f"P_DG_29[{t}]"]) for t in range(window_time)],
            "P_DG_64": [float(dict_value[f"P_DG_64[{t}]"]) for t in range(window_time)],
            "SOC_BSS_12": [float(dict_value[f"SOC_BSS_12[{t}]"]) for t in range(window_time)],
            "SOC_BSS_34": [float(dict_value[f"SOC_BSS_34[{t}]"]) for t in range(window_time)],
            "SOC_BSS_68": [float(dict_value[f"SOC_BSS_68[{t}]"]) for t in range(window_time)],
            "SOC_BSS_103": [float(dict_value[f"SOC_BSS_103[{t}]"]) for t in range(window_time)],
            "runtime": float(model.Runtime),
            "model_stats": globals().get("collect_gurobi_model_stats", lambda _model: {})(model),
        }
...
def _get_scenario():
    ...
    wind_11_avail_list = data["wind_11"]
    wind_32_avail_list = data["wind_32"]
    wind_66_avail_list = data["wind_66"]
    wind_101_avail_list = data["wind_101"]
    pv_22_avail_list = data["pv_22"]
    pv_36_avail_list = data["pv_36"]
    pv_70_avail_list = data["pv_70"]
    pv_107_avail_list = data["pv_107"]
    load_list = data["load"]
    load_list_Q = data["load_Q"]
...
for seed in range(5,105):
    ...
    MPC = Solve_MPC(load_min, load_max, list_r_x_pu, list_p_q_pu, load_min_Q, load_max_Q)
    ...
    (obj, new_P_DG_29_init, new_P_DG_64_init,
         ...
         cost_hour_now, solve_time) \
            = MPC.sol_pro_all_day(window_time,
                    load_list, load_list_Q,
                    P_DG_29_init, P_DG_64_init,
                    SOC_BSS_12_init, SOC_BSS_34_init, SOC_BSS_68_init, SOC_BSS_103_init,
                    wind_11_avail_list, wind_32_avail_list, wind_66_avail_list, wind_101_avail_list,
