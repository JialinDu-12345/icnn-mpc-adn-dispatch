...
class PowerSystemEnv(gym.Env):
    def __init__(self):
        ...
        print("ori_load", list(self.ori_load))

        self.ori_load_Q = self.net.load.q_mvar.values.copy()

        print("ori_load_Q", list(self.ori_load_Q))
        ...
        self.wind_Q_a_min = -1.35
        self.wind_Q_a_max = 1.35
        self.wind_Q_a_scale = 1.35

        self.wind_Q_b_min = -1
        self.wind_Q_b_max = 1
        self.wind_Q_b_scale = 1
        ...
        self.observation_space = spaces.Box(
            ...
            shape=(1 + self.num_load_buses + self.num_load_buses + NUM_WT_a + NUM_WT_b + NUM_GEN + NUM_BSS,),
            dtype=np.float32)

    def _adjust_network_119(self):
        self.net.sn_mva = 1

        bus0 = pp.create_bus(self.net, vn_kv=12.66, zone=1.0, max_vm_pu=1.1, min_vm_pu=0.9, name="0")
        ...
        for i in range(118):
            pp.create_bus(self.net, vn_kv=12.66, zone=1.0, max_vm_pu=1.1, min_vm_pu=0.9, name='{}'.format(i + 1))
        ...
        for i in range(118):
            list_pq[i][0] = list_pq[i][0] / 3000
            list_pq[i][1] = list_pq[i][1] / 3000
    ...
    def _adjust_network(self):
        ...
        pp.create_sgen(self.net, bus=10, p_mw=0, q_mvar=0, name='WT_a_11')
        pp.create_sgen(self.net, bus=100, p_mw=0, q_mvar=0, name='WT_a_101')

        pp.create_sgen(self.net, bus=31, p_mw=0, q_mvar=0, name='WT_b_32')
        pp.create_sgen(self.net, bus=65, p_mw=0, q_mvar=0, name='WT_b_66')

    def _create_load_profile(self):
        a0_1 = np.random.uniform(0.6, 1, (6, self.num_load_buses))
        a0_2 = np.random.uniform(0.8, 1.2, (6, self.num_load_buses))
        a0_3 = np.random.uniform(1, 1.4, (6, self.num_load_buses))
        a0_4 = np.random.uniform(0.8, 1.2, (6, self.num_load_buses))
        ...
        a0_1_Q = np.random.uniform(0.8, 1, (6, self.num_load_buses))
        a0_2_Q = np.random.uniform(0.9, 1.1, (6, self.num_load_buses))
        a0_3_Q = np.random.uniform(1, 1.2, (6, self.num_load_buses))
        a0_4_Q = np.random.uniform(0.9, 1.1, (6, self.num_load_buses))
    ...
    def _create_renew_profile(self):
        return {
            'wind_a_11': np.random.uniform(1.2, 1.8, 24),
            'wind_a_101': np.random.uniform(1.2, 1.8, 24),
            'wind_b_32': np.random.uniform(0.8, 1.2, 24),
            'wind_b_66': np.random.uniform(0.8, 1.2, 24),
        }
    ...
    def new_reset(self, list_feng_11, list_feng_101, list_feng_32, list_feng_66,
        ...
        self.renew_profile = {
            'wind_a_11': list_feng_11,
            'wind_a_101': list_feng_101,
            'wind_b_32': list_feng_32,
            'wind_b_66': list_feng_66,
        }
    ...
    def _get_state(self):
        ...
        renew_pred = [self.renew_profile['wind_a_11'][self.hour],
                      self.renew_profile['wind_a_101'][self.hour],
                      self.renew_profile['wind_b_32'][self.hour],
                      self.renew_profile['wind_b_66'][self.hour]].copy()
        ...
        wind_a_11_normalized = (renew_pred[0] - self.wind_a_min) / (self.wind_a_max - self.wind_a_min + 1e-6)
        wind_a_101_normalized = (renew_pred[1] - self.wind_a_min) / (self.wind_a_max - self.wind_a_min + 1e-6)
        wind_b_32_normalized = (renew_pred[2] - self.wind_b_min) / (self.wind_b_max - self.wind_b_min + 1e-6)
        wind_b_66_normalized = (renew_pred[3] - self.wind_b_min) / (self.wind_b_max - self.wind_b_min + 1e-6)

        state = np.concatenate([
            [t_normalized],
            load_normalized,
            load_Q_normalized,
            [wind_a_11_normalized, wind_a_101_normalized],
            [wind_b_32_normalized, wind_b_66_normalized],
    ...
    def step(self, action):
        ...
        gen_29_delta = get_action[0] * self.gen_ramp
        gen_64_delta = get_action[1] * self.gen_ramp
        ...
        SOC_12_delta = get_action[6] * self.SOC_scale
        SOC_34_delta = get_action[7] * self.SOC_scale
        SOC_68_delta = get_action[8] * self.SOC_scale
        SOC_103_delta = get_action[9] * self.SOC_scale
        ...
        if self.SOC_12 - new_SOC_12 >= 0:
            new_BSS_12_p = (self.SOC_12 - new_SOC_12) * 0.98
        else:
            new_BSS_12_p = (self.SOC_12 - new_SOC_12) * 1.02
        if self.SOC_34 - new_SOC_34 >= 0:
            new_BSS_34_p = (self.SOC_34 - new_SOC_34) * 0.98
        else:
            new_BSS_34_p = (self.SOC_34 - new_SOC_34) * 1.02
        if self.SOC_68 - new_SOC_68 >= 0:
            new_BSS_68_p = (self.SOC_68 - new_SOC_68) * 0.98
        else:
            new_BSS_68_p = (self.SOC_68 - new_SOC_68) * 1.02
        if self.SOC_103 - new_SOC_103 >= 0:
            new_BSS_103_p = (self.SOC_103 - new_SOC_103) * 0.98
        else:
            new_BSS_103_p = (self.SOC_103 - new_SOC_103) * 1.02

        new_WT_a_11_Q = get_action[10] * self.wind_Q_a_scale
        new_WT_a_101_Q = get_action[11] * self.wind_Q_a_scale

        new_WT_b_32_Q = get_action[12] * self.wind_Q_b_scale
        new_WT_b_66_Q = get_action[13] * self.wind_Q_b_scale

        wind_a_11_avail = self.renew_profile['wind_a_11'][self.hour].copy()
        wind_a_101_avail = self.renew_profile['wind_a_101'][self.hour].copy()

        wind_b_32_avail = self.renew_profile['wind_b_32'][self.hour].copy()
        wind_b_66_avail = self.renew_profile['wind_b_66'][self.hour].copy()
        ...
        self.net.sgen.p_mw = np.hstack(([new_gen_29_p, new_gen_64_p],
                                        ...
                                        [wind_a_11_avail, wind_a_101_avail],
                                        [wind_b_32_avail, wind_b_66_avail]))

        self.net.sgen.q_mvar = np.hstack(([0, 0],
                                          [new_SVC_20, new_SVC_56, new_SVC_79, new_SVC_105],
                                          [0, 0, 0, 0],
                                          [new_WT_a_11_Q, new_WT_a_101_Q],
                                          [new_WT_b_32_Q, new_WT_b_66_Q]))
        ...
        balance_penalty = Price_current * max(0, balance_error)

        p_29 = self.net.sgen.p_mw[0]
        cost_29 = 0.00240 * p_29 ** 2 + 12.3299 * p_29 + 28
        p_64 = self.net.sgen.p_mw[1]
        cost_64 = 0.00240 * p_64 ** 2 + 12.3299 * p_64 + 28
        cost = cost_29 + cost_64

        reward = - (cost + balance_penalty) / 500
    ...
    def step_1(self, action):
        ...
        self.net.sgen.p_mw = np.hstack(
            ([new_gen_29_p, new_gen_64_p],
             [0, 0, 0, 0],
             [new_BSS_12_p, new_BSS_34_p, new_BSS_68_p, new_BSS_103_p],
             [wind_a_11_avail, wind_a_101_avail],
             [wind_b_32_avail, wind_b_66_avail]))
...
def _test_agent(env, agent, list_feng_41, list_feng_48, list_feng_69, list_feng_90,
                list_feng_6, list_feng_13, list_feng_107, list_feng_115,
    ...
    state = env.new_reset(list_feng_41, list_feng_48, list_feng_69, list_feng_90,
                          list_feng_6, list_feng_13, list_feng_107, list_feng_115,
                          list_fu_he, list_fu_he_Q)
    done = False
...
def _test_agent_1(env, agent, list_feng_41, list_feng_48, list_feng_69, list_feng_90,
                list_feng_6, list_feng_13, list_feng_107, list_feng_115,
                  list_fu_he, list_fu_he_Q, model_path='model.pth'):
    agent.actor.load_state_dict(torch.load(model_path))
