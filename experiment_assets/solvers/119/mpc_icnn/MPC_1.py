...
class Solve_MPC:
    def __init__(self, Q_1_mat, Q_2_mat, load_min, load_max, list_r_x_pu, list_p_q_pu, load_min_Q, load_max_Q):
        ...
        self.scale_Q = 0.3
        self.clip_icnn_exogenous_inputs = True
        self.icnn_exogenous_clip_lb = -1.0
        self.icnn_exogenous_clip_ub = 1.0
        self.icnn_clip_count_last_solve = 0
        self.icnn_clip_max_violation_last_solve = 0.0
    ...
    def _add_icnn_input_var(self, model, name):
        return model.addVar(lb=-1.0, ub=1.0, vtype=GRB.CONTINUOUS, name=name)

    def _add_icnn_input_mvar(self, model, shape, name):
        return model.addMVar(shape=shape, lb=-1.0, ub=1.0, vtype=GRB.CONTINUOUS, name=name)

    def _clip_icnn_exogenous_value(self, value, lb=None, ub=None):
        value = float(value)
        if lb is None:
            lb = float(getattr(self, "icnn_exogenous_clip_lb", -1.0))
        if ub is None:
            ub = float(getattr(self, "icnn_exogenous_clip_ub", 1.0))
        if not getattr(self, "clip_icnn_exogenous_inputs", True):
            return value

        clipped = float(np.clip(value, lb, ub))
        violation = abs(clipped - value)
        if violation > 1e-9:
            self.icnn_clip_count_last_solve = int(
                getattr(self, "icnn_clip_count_last_solve", 0)
            ) + 1
            self.icnn_clip_max_violation_last_solve = max(
                float(getattr(self, "icnn_clip_max_violation_last_solve", 0.0)),
                violation,
            )
        return clipped

    def _reset_icnn_clip_stats(self):
        self.icnn_clip_count_last_solve = 0
        self.icnn_clip_max_violation_last_solve = 0.0
    ...
    def sol_pro(self, current_time, window_time,
        ...
        try:
            ...
            self._reset_icnn_clip_stats()

            model.setParam('MIPFocus',0)

            z_0_1 = model.addMVar(shape=(1, 269), lb=-GRB.INFINITY, ub=GRB.INFINITY, vtype=GRB.CONTINUOUS, name='z_0_1')
            ...
            z_0_2 = model.addMVar(shape=(1, 269), lb=-GRB.INFINITY, ub=GRB.INFINITY, vtype=GRB.CONTINUOUS, name='z_0_2')
            ...
            load_normal_st = self._add_icnn_input_mvar(model, shape=(118,), name='load_normal_st')
            load_normal_st_Q = self._add_icnn_input_mvar(model, shape=(118,), name='load_normal_st_Q')

            P_WT_11_normal_st = self._add_icnn_input_var(model, 'P_WT_11_normal_st')
            P_WT_32_normal_st = self._add_icnn_input_var(model, 'P_WT_32_normal_st')
            P_WT_66_normal_st = self._add_icnn_input_var(model, 'P_WT_66_normal_st')
            P_WT_101_normal_st = self._add_icnn_input_var(model, 'P_WT_101_normal_st')

            P_PV_22_normal_st = self._add_icnn_input_var(model, 'P_PV_22_normal_st')
            P_PV_36_normal_st = self._add_icnn_input_var(model, 'P_PV_36_normal_st')
            P_PV_70_normal_st = self._add_icnn_input_var(model, 'P_PV_70_normal_st')
            P_PV_107_normal_st = self._add_icnn_input_var(model, 'P_PV_107_normal_st')

            P_DG_29_normal_st_1 = self._add_icnn_input_var(model, 'P_DG_29_normal_st_1')
            P_DG_64_normal_st_1 = self._add_icnn_input_var(model, 'P_DG_64_normal_st_1')

            P_BSS_12_normal_st_1 = self._add_icnn_input_var(model, 'P_BSS_12_normal_st_1')
            P_BSS_34_normal_st_1 = self._add_icnn_input_var(model, 'P_BSS_34_normal_st_1')
            P_BSS_68_normal_st_1 = self._add_icnn_input_var(model, 'P_BSS_68_normal_st_1')
            P_BSS_103_normal_st_1 = self._add_icnn_input_var(model, 'P_BSS_103_normal_st_1')

            Actor_DG_29_st = self._add_icnn_input_var(model, 'Actor_DG_29_st')
            Actor_DG_64_st = self._add_icnn_input_var(model, 'Actor_DG_64_st')
            Actor_SVC_20_st = self._add_icnn_input_var(model, 'Actor_SVC_20_st')
            Actor_SVC_56_st = self._add_icnn_input_var(model, 'Actor_SVC_56_st')
            Actor_SVC_79_st = self._add_icnn_input_var(model, 'Actor_SVC_79_st')
            Actor_SVC_105_st = self._add_icnn_input_var(model, 'Actor_SVC_105_st')
            Actor_SOC_12_st = self._add_icnn_input_var(model, 'Actor_SOC_12_st')
            Actor_SOC_34_st = self._add_icnn_input_var(model, 'Actor_SOC_34_st')
            Actor_SOC_68_st = self._add_icnn_input_var(model, 'Actor_SOC_68_st')
            Actor_SOC_103_st = self._add_icnn_input_var(model, 'Actor_SOC_103_st')
            Actor_WT_11_st = self._add_icnn_input_var(model, 'Actor_WT_11_st')
            Actor_WT_32_st = self._add_icnn_input_var(model, 'Actor_WT_32_st')
            Actor_WT_66_st = self._add_icnn_input_var(model, 'Actor_WT_66_st')
            Actor_WT_101_st = self._add_icnn_input_var(model, 'Actor_WT_101_st')
            Actor_PV_22_st = self._add_icnn_input_var(model, 'Actor_PV_22_st')
            Actor_PV_36_st = self._add_icnn_input_var(model, 'Actor_PV_36_st')
            Actor_PV_70_st = self._add_icnn_input_var(model, 'Actor_PV_70_st')
            Actor_PV_107_st = self._add_icnn_input_var(model, 'Actor_PV_107_st')
            ...
            for i in range(118):
                ...
                model.addConstr(
                    load_normal_st[i] == self._clip_icnn_exogenous_value(load_norm_i)
                )
                model.addConstr(
                    load_normal_st_Q[i] == self._clip_icnn_exogenous_value(load_q_norm_i)
                )
            wt_11_norm = (
                self.wind_11_avail_list[terminal_idx] - self.wind_min
            ) / (self.wind_max - self.wind_min + 1e-6)
            wt_32_norm = (
                self.wind_32_avail_list[terminal_idx] - self.wind_min
            ) / (self.wind_max - self.wind_min + 1e-6)
            wt_66_norm = (
                self.wind_66_avail_list[terminal_idx] - self.wind_min
            ) / (self.wind_max - self.wind_min + 1e-6)
            wt_101_norm = (
                self.wind_101_avail_list[terminal_idx] - self.wind_min
            ) / (self.wind_max - self.wind_min + 1e-6)
            pv_22_norm = (
                self.pv_22_avail_list[terminal_idx] - self.pv_min
            ) / (self.pv_max - self.pv_min + 1e-6)
            pv_36_norm = (
                self.pv_36_avail_list[terminal_idx] - self.pv_min
            ) / (self.pv_max - self.pv_min + 1e-6)
            pv_70_norm = (
                self.pv_70_avail_list[terminal_idx] - self.pv_min
            ) / (self.pv_max - self.pv_min + 1e-6)
            pv_107_norm = (
                self.pv_107_avail_list[terminal_idx] - self.pv_min
            ) / (self.pv_max - self.pv_min + 1e-6)
            model.addConstr(P_WT_11_normal_st == self._clip_icnn_exogenous_value(wt_11_norm))
            model.addConstr(P_WT_32_normal_st == self._clip_icnn_exogenous_value(wt_32_norm))
            model.addConstr(P_WT_66_normal_st == self._clip_icnn_exogenous_value(wt_66_norm))
            model.addConstr(P_WT_101_normal_st == self._clip_icnn_exogenous_value(wt_101_norm))
            model.addConstr(P_PV_22_normal_st == self._clip_icnn_exogenous_value(pv_22_norm))
            model.addConstr(P_PV_36_normal_st == self._clip_icnn_exogenous_value(pv_36_norm))
            model.addConstr(P_PV_70_normal_st == self._clip_icnn_exogenous_value(pv_70_norm))
            model.addConstr(P_PV_107_normal_st == self._clip_icnn_exogenous_value(pv_107_norm))
            model.addConstr(P_DG_29_normal_st_1 == (P_DG_29[window_time - 2] - self.gen_min)
                            / (self.gen_max - self.gen_min + 1e-6))
            model.addConstr(P_DG_64_normal_st_1 == (P_DG_64[window_time - 2] - self.gen_min)
                            / (self.gen_max - self.gen_min + 1e-6))
            model.addConstr(P_BSS_12_normal_st_1 == (SOC_BSS_12[window_time - 2] - self.SOC_min)
                            / (self.SOC_max - self.SOC_min + 1e-6))
            model.addConstr(P_BSS_34_normal_st_1 == (SOC_BSS_34[window_time - 2] - self.SOC_min)
                            / (self.SOC_max - self.SOC_min + 1e-6))
            model.addConstr(P_BSS_68_normal_st_1 == (SOC_BSS_68[window_time - 2] - self.SOC_min)
                            / (self.SOC_max - self.SOC_min + 1e-6))
            model.addConstr(P_BSS_103_normal_st_1 == (SOC_BSS_103[window_time - 2] - self.SOC_min)
                            / (self.SOC_max - self.SOC_min + 1e-6))
            ...
            for i in range(118):
                model.addConstr(z0[i+119] == load_normal_st_Q[i])
            model.addConstr(z0[237] == P_WT_11_normal_st)
            model.addConstr(z0[238] == P_WT_32_normal_st)
            model.addConstr(z0[239] == P_WT_66_normal_st)
            model.addConstr(z0[240] == P_WT_101_normal_st)
            model.addConstr(z0[241] == P_PV_22_normal_st)
            model.addConstr(z0[242] == P_PV_36_normal_st)
            model.addConstr(z0[243] == P_PV_70_normal_st)
            model.addConstr(z0[244] == P_PV_107_normal_st)
            model.addConstr(z0[245] == P_DG_29_normal_st_1)
            model.addConstr(z0[246] == P_DG_64_normal_st_1)
            model.addConstr(z0[247] == P_BSS_12_normal_st_1)
            model.addConstr(z0[248] == P_BSS_34_normal_st_1)
            model.addConstr(z0[249] == P_BSS_68_normal_st_1)
            model.addConstr(z0[250] == P_BSS_103_normal_st_1)

            model.addConstr(z0[251] == Actor_DG_29_st)
            model.addConstr(z0[252] == Actor_DG_64_st)
            model.addConstr(z0[253] == Actor_SVC_20_st)
            model.addConstr(z0[254] == Actor_SVC_56_st)
            model.addConstr(z0[255] == Actor_SVC_79_st)
            model.addConstr(z0[256] == Actor_SVC_105_st)
            model.addConstr(z0[257] == Actor_SOC_12_st)
            model.addConstr(z0[258] == Actor_SOC_34_st)
            model.addConstr(z0[259] == Actor_SOC_68_st)
            model.addConstr(z0[260] == Actor_SOC_103_st)
            model.addConstr(z0[261] == Actor_WT_11_st)
            model.addConstr(z0[262] == Actor_WT_32_st)
            model.addConstr(z0[263] == Actor_WT_66_st)
            model.addConstr(z0[264] == Actor_WT_101_st)
            model.addConstr(z0[265] == Actor_PV_22_st)
            model.addConstr(z0[266] == Actor_PV_36_st)
            model.addConstr(z0[267] == Actor_PV_70_st)
            model.addConstr(z0[268] == Actor_PV_107_st)
...
seedlist = [88,89,91,92,94,95,96,98,99,100,101]
objlist = []
timelist = []
