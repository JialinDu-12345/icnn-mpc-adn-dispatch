...
DEFAULT_TASKS = ["MPC-3", "SAC", "Proposed-H3"]
...
def parse_tasks(raw: str) -> List[str]:
    ...
    for item in [part.strip() for part in raw.split(",") if part.strip()]:
        ...
        if task not in DEFAULT_TASKS:
            raise ValueError(f"Unknown task '{item}'. Expected one of {DEFAULT_TASKS}.")
        if task in seen:
            continue
        tasks.append(task)
        seen.add(task)
...
def build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Run one 33-bus scenario for paper dispatch-trajectory plots."
    )
    parser.add_argument("--scenario", type=int, default=80)
    parser.add_argument("--tasks", default="MPC-3,SAC,Proposed-H3")
    parser.add_argument("--forecast-case", default="id_reliable")
    parser.add_argument("--actual-case", default="id_actual")
    parser.add_argument("--gamma-Q", default="auto")
    ...
    parser.add_argument("--output", default="formal/step9_33_dispatch_trajectory_raw.csv")
    return parser
...
def failure_row(task: str, args: argparse.Namespace, exc: BaseException) -> Dict[str, Any]:
    ...
    forecast_case = args.forecast_case if task != "SAC" else "not_applicable"
    return make_result_row(
        ...
        seed=DEFAULT_BASE_SEEDS["33"] + int(args.scenario),
        executed_steps=0,
...
def main(argv: Optional[Iterable[str]] = None) -> None:
    ...
    policy_adapter: Optional[PolicyAdapter33] = None
    ...
    for task in tasks:
        try:
            if task == "MPC-3":
                ...
                if policy_adapter is None:
                    policy_adapter = PolicyAdapter33(
                        method="SAC",
                        device=args.device,
                        quiet_legacy_init=not args.verbose_legacy_init,
                    )
                row = policy_adapter.run_one_day(
                    scenario_id=args.scenario,
                    actual_case=args.actual_case,
                    max_steps=args.max_steps,
        ...
        row["experiment_tag"] = "dispatch_trajectory_plot"
