...
GROUP_COLS = ["system", "actual_case", "forecast_case"]
...
SUMMARY_FIELDS = [
    *GROUP_COLS,
    ...
    "std_cost_reduction_percent",
    "ci95_cost_reduction_percent",
    "mean_time_diff_s",
    "std_time_diff_s",
    "ci95_time_diff_s",
    "mean_time_reduction_percent",
    "std_time_reduction_percent",
    "ci95_time_reduction_percent",
]
...
def group_key_from_detail(row: Dict[str, object]) -> Tuple[str, ...]:
    return tuple(str(row.get(col, "")) for col in GROUP_COLS)


def build_completed_lookup(rows: List[Dict[str, str]]) -> Dict[Tuple[str, ...], Dict[str, Dict[str, str]]]:
    ...
    for row in rows:
        ...
        if not method:
            continue
        lookup.setdefault(scenario_key(row), {})[method] = row
    return lookup


def build_pairwise_details(
        ...
        details.append(
            {
                **dict(zip(INDEX_COLS, key)),
                ...
                "time_diff_s": target_time - baseline_time,
...
def summarize_by_case(
    details: List[Dict[str, object]], baseline: str, target: str
) -> List[Dict[str, object]]:
    grouped: Dict[Tuple[str, ...], List[Dict[str, object]]] = defaultdict(list)
    for row in details:
        grouped[group_key_from_detail(row)].append(row)

    summary_rows: List[Dict[str, object]] = []
    for key in sorted(grouped):
        group = grouped[key]
        key_dict = dict(zip(GROUP_COLS, key))
        cost_diffs = [parse_float(row.get("cost_diff")) for row in group]
        cost_reductions = [parse_float(row.get("cost_reduction_percent")) for row in group]
        time_diffs = [parse_float(row.get("time_diff_s")) for row in group]
        time_reductions = [parse_float(row.get("time_reduction_percent")) for row in group]
...
def build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Matched-pair summary grouped by forecast case.")
    parser.add_argument("--input", default="formal/step5_33_forecast_sensitivity_raw.csv")
    parser.add_argument("--output", default="formal/step5_33_pairwise_mpc3_vs_proposed_by_case.csv")
...
def main(argv: Optional[Iterable[str]] = None) -> None:
    ...
    details = build_pairwise_details(rows, args.baseline, args.target)
    summary_rows = summarize_by_case(details, args.baseline, args.target)
    ...
    write_rows(output_path, summary_rows, SUMMARY_FIELDS)
